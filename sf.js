...
function normalizeTieId(league, a, b) {
  ...
  return `${league}-semifinal-${p}__${q}`.replace(/[^a-z0-9_-]/g, '_');
}
...
async function loadExistingSemiFinalTies(matchColl) {
  const q = query(
...
async function loadEligibleTeams() {
  ...
  log(`Loading existing Semi Final ties from ${matchColl}…`);
  const existingTies = await loadExistingSemiFinalTies(matchColl);
  log(`Found ${existingTies.length} existing Semi Final ties.`, existingTies.length ? 'ok' : 'warn');

  // render 2 matches (4 teams total)
  for (let i = 0; i < 2; i++) {
    matchesEl.appendChild(renderMatchRow(i, matchColl));
  }

  // lock rows for existing ties (up to 2)
  const usedByExisting = new Set();
  for (let i = 0; i < Math.min(2, existingTies.length); i++) {
