...
function normalizeTieId(league, a, b) {
  ...
  return `${league}-quarterfinal-${p}__${q}`.replace(/[^a-z0-9_-]/g, '_');
}
...
async function loadExistingQuarterFinalTies(matchColl) {
  const q = query(
...
async function loadEligibleTeams() {
  ...
  // Note: Adjust filter here if you have specific requirements.
  ...
  log(`Loading existing Quarter Final ties from ${matchColl}…`);
  const existingTies = await loadExistingQuarterFinalTies(matchColl);
  log(`Found ${existingTies.length} existing Quarter Final ties.`, existingTies.length ? 'ok' : 'warn');

  // render 4 matches (8 teams total)
  for (let i = 0; i < 4; i++) {
    matchesEl.appendChild(renderMatchRow(i, matchColl));
  }

  // lock rows for existing ties (up to 4)
  const usedByExisting = new Set();
  for (let i = 0; i < Math.min(4, existingTies.length); i++) {
