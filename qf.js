...
if (!league) {
    ...
    document.getElementById("league-title").innerText = `${league} Quarterfinal Matches`;
    loadMatches();
}
...
async function loadMatches() {
    try {
        const matchesSnapshot = await db.collection("quarterfinalmatches")
            .where("league", "==", league)
            .get();
...
async function loadTeams(existingMatches) {
    try {
        ...
        teamsSnapshot.forEach(doc => {
            ...
            // ✅ Only include teams not eliminated
    ...
    for (let i = 0; i < 4; i++) {
        ...
        if (!existingMatch1) {
            match1Div.querySelector(".set-match").addEventListener("click", async function () {
                ...
                try {
                    const match1Ref = await db.collection("quarterfinalmatches").add({
                        league: league,
                        matchday: 1,
                        matchIndex: i,
                        team1: team1,
                    ...
                    const match2Ref = await db.collection("quarterfinalmatches").add({
                        league: league,
                        matchday: 2,
                        matchIndex: i,
                        team1: team2,
        ...
        match1Div.querySelector(".set-winner").addEventListener("click", async function () {
            ...
            await db.collection("quarterfinalmatches").doc(existingMatch1?.id).update({ winner });
        ...
        match2Div.querySelector(".set-winner").addEventListener("click", async function () {
            ...
            await db.collection("quarterfinalmatches").doc(existingMatch2?.id).update({ winner });
...
async function updateEndgameMatchPoints() {
    try {
        const collections = ["roundof16matches", "quarterfinalmatches", "playoffmatches"];
        let teamPoints = {};
